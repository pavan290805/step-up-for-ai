...
const ROLES: RoleConfig[] = [
  {
    ...
    imageSrc: "/images/auth/student-auth.png",
  },
  {
    role: "recruiter",
    label: "Recruiter",
    ...
    imageSrc: "/images/auth/recruiter-auth.png",
  },
  {
    role: "investor",
    label: "Investor",
    ...
    imageSrc: "/images/auth/investor-auth.png",
  },
  {
    role: "admin",
    label: "Admin",
    ...
    imageSrc: "/images/auth/admin-auth.png",
  },
];
...
function HeroPanel({ config, activeRole, setActiveRole }: { config: RoleConfig; activeRole: Role; setActiveRole: (role: Role) => void }) {
  return (
    <section className="hero-panel">
      <img className="hero-image" src={config.imageSrc} alt={`${config.label} authentication illustration`} />

      <div className="role-switch hero-role-switch" aria-label="Choose role">
        {ROLES.map((role) => (
...
function AuthPanel({ config, mode, formData, handleInputChange, handleSubmit, setActiveRole }: { config: RoleConfig; mode: Mode; formData: Record<string, string>; handleInputChange: (e: React.ChangeEvent<HTMLInputElement>) => void; handleSubmit: (e: React.FormEvent) => void; setActiveRole: (role: Role) => void }) {
  ...
  return (
    <section className="auth-panel">
      <div className="role-switch form-role-switch" aria-label="Choose role">
        {ROLES.map((role) => (
          <button key={role.role} type="button" onClick={() => setActiveRole(role.role)} className={config.role === role.role ? "active" : ""} style={{ "--role-accent": role.accent } as React.CSSProperties}>
            {role.shortLabel}
          </button>
        ))}
      </div>
      ...
      <div className="tabs">
        <Link to="/login" className={mode === "login" ? "selected" : ""}>Login</Link>
        <Link to="/signup" className={mode === "signup" ? "selected" : ""}>Sign Up</Link>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        {mode === "login" ? (
          ...
          <label className="terms"><input type="checkbox" /> <span>I agree to the <Link to="#">Terms &amp; Conditions</Link></span></label>
        )}
    ...
    <main className="auth-shell" style={{ "--accent": config.accent, "--accent-soft": config.accentSoft, "--hero-bg": config.panelBg, "--hero-glow": config.glow } as React.CSSProperties}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        body { margin: 0; }
        .auth-shell { width: 100vw; min-height: 100vh; display: grid; place-items: stretch; font-family: Inter, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; color: #f8fafc; background: #02040a; overflow-x: hidden; }
        .auth-layout { width: 100vw; height: 100vh; min-height: 100vh; display: grid; grid-template-columns: 50vw 50vw; overflow: hidden; background: #02040a; }
        .hero-panel { position: relative; min-height: 100vh; height: 100vh; width: 50vw; padding: 48px; overflow: hidden; display: flex; align-items: center; justify-content: center; background: radial-gradient(circle at 24% 20%, color-mix(in srgb, var(--accent) 18%, transparent), transparent 34%), var(--hero-bg); }
        .hero-image { display: block; max-width: 85%; max-height: 85%; width: auto; height: auto; object-fit: contain; object-position: center; filter: drop-shadow(0 26px 60px rgba(0, 0, 0, .32)); }
        .hero-copy, .stat-row, .hero-visual { position: relative; z-index: 1; }
        ...
        .hero-role-switch { position: absolute; left: 48px; bottom: 40px; z-index: 2; max-width: calc(100% - 96px); }
        .form-role-switch { margin-bottom: 22px; max-width: 100%; }
        ...
        .auth-panel { position: relative; z-index: 2; align-self: center; justify-self: center; width: min(100%, 460px); max-width: 460px; padding: 32px; border-radius: 8px; border: 1px solid color-mix(in srgb, var(--accent) 24%, rgba(148, 163, 184, .13)); background: rgba(7, 11, 20, .76); box-shadow: 0 22px 58px rgba(0, 0, 0, .42), 0 0 40px color-mix(in srgb, var(--accent) 12%, transparent), inset 0 1px 0 rgba(255,255,255,.035); backdrop-filter: blur(18px); }
        ...
        @media (min-width: 1024px) { .auth-layout { grid-template-columns: 50vw 50vw; } .hero-panel { width: 50vw; } }
        @media (max-width: 1023px) and (min-width: 721px) { .auth-layout { grid-template-columns: 55vw 45vw; } .hero-panel { width: 55vw; padding: 40px; } .auth-panel { width: min(100%, 420px); padding: 28px; } }
        @media (max-width: 720px) { .auth-layout { width: 100%; min-height: 100vh; height: auto; grid-template-columns: 1fr; grid-template-rows: minmax(280px, 38vh) auto; overflow: visible; } .hero-panel { width: 100%; min-height: 280px; height: 38vh; padding: 28px; } .hero-image { max-width: 88%; max-height: 82%; } .hero-role-switch { display: none; } .auth-panel { width: min(460px, calc(100% - 32px)); margin: 26px auto 34px; padding: 26px 20px; } .role-switch { width: 100%; } }
      `}</style>
      <div className="auth-layout">
        <HeroPanel config={config} activeRole={activeRole} setActiveRole={setActiveRole} />
        <AuthPanel config={config} mode={mode} formData={formData} handleInputChange={handleInputChange} handleSubmit={handleSubmit} setActiveRole={setActiveRole} />
      </div>
    </main>
  );
}
