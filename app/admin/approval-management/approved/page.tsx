...
export default function ApprovedAccountsPage() {
  const [search, setSearch] = useState("");

  const approvedRecs = mockRecruiters.filter((r) => r.status === "Approved").map((r) => ({ ...r, type: "Recruiter" }));
  const approvedInvs = mockInvestors.filter((i) => i.status === "Approved").map((i) => ({ ...i, type: "Investor", company: i.organization }));
  
  const allApproved = [...approvedRecs, ...approvedInvs];

  const filtered = allApproved.filter((item) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        ...
        description="Verify and browse the directory of all platform-approved recruiter and investor profiles."
      />
      ...
      {/* Table */}
      <Table headers={["ID", "Name", "Role / Type", "Organization / Company", "Designation", "Email", "Approval Date", "Status"]}>
        {filtered.map((item) => (
