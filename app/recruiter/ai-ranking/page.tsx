...
function RankBadge({ rank }: { rank: number }) {
  ...
  const icons = ['', '', ''];
  return (
    <span style={{ fontSize: rank <= 3 ? 22 : 14, color: rank <= 3 ? colors[rank - 1] : '#6b5d85', fontWeight: 700, minWidth: 32, textAlign: 'center' }}>
  ...
  return (
    <>
      <style>{`
        ...
        .ar-btn { padding: 10px 22px; border-radius: 11px; background: linear-gradient(135deg,#a855f7,#7c3aed); color:#fff; font: 700 13px Inter; border: none; cursor: pointer; transition: all .2s; box-shadow: 0 6px 16px rgba(168, 85, 247,.3); }
        .ar-btn:hover { transform: translateY(-1px); }
        .ar-list { display: flex; flex-direction: column; gap: 14px; }
        .ar-card { background: linear-gradient(145deg,rgba(13, 7, 24,.9),rgba(8, 4, 26,.9)); border: 1px solid rgba(168, 85, 247,.15); border-radius: 20px; padding: 22px 24px; display: flex; align-items: flex-start; gap: 18px; transition: all .2s; }
        .ar-card:hover { border-color: rgba(168, 85, 247,.35); transform: translateY(-2px); }
        .ar-card.top { border-color: rgba(251,191,36,.3); background: linear-gradient(145deg,rgba(13, 7, 24,.9),rgba(251,191,36,.04)); }
        .ar-rank-col { display: flex; align-items: center; justify-content: center; min-width: 36px; padding-top: 4px; }
        .ar-av { width: 46px; height: 46px; border-radius: 13px; background: linear-gradient(135deg,#a855f7,#7c3aed); display:grid; place-items:center; font-size:16px; font-weight:700; color:#fff; flex-shrink:0; }
        .ar-info { flex: 1; min-width: 0; }
        .ar-name { font-size: 15px; font-weight: 700; color: #f1eeff; margin-bottom: 2px; }
        .ar-meta { font-size: 12px; color: #7c6d9e; margin-bottom: 12px; }
        .ar-bars { display: flex; flex-direction: column; gap: 7px; margin-bottom: 12px; }
        .ar-bar-label { font-size: 11px; color: #9d7ec7; margin-bottom: 3px; }
        .ar-reason { font-size: 12px; color: #9d7ec7; line-height: 1.6; background: rgba(168, 85, 247,.06); border: 1px solid rgba(168, 85, 247,.12); border-radius: 10px; padding: 10px 13px; margin-top: 4px; }
        .ar-reason-icon { font-size: 14px; margin-right: 4px; }
        .ar-score-bubble { flex-shrink: 0; text-align: center; background: linear-gradient(135deg,rgba(168, 85, 247,.18),rgba(124, 58, 237,.12)); border: 1px solid rgba(168, 85, 247,.3); border-radius: 14px; padding: 14px 16px; }
        .ar-score-val { font-size: 28px; font-weight: 800; color: #c084fc; line-height: 1; }
        .ar-score-label { font-size: 10px; color: #7c6d9e; margin-top: 4px; font-weight: 600; text-transform: uppercase; letter-spacing: .04em; }
      `}</style>
            ...
            <div className={`ar-card${c.rank === 1 ? ' top' : ''}`} key={c.rank}>
              ...
              <div className="ar-info">
                ...
                <div className="ar-bars">
                  <div><div className="ar-bar-label">Skills Match</div><ScoreBar value={c.skills} color="#a855f7" /></div>
                  <div><div className="ar-bar-label">Experience</div><ScoreBar value={c.exp} color="#818cf8" /></div>
                  <div><div className="ar-bar-label">Culture Fit</div><ScoreBar value={c.culture} color="#c084fc" /></div>
                </div>
